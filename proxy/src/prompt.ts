...
export function buildPrompt(
  ...
  const hospitalNameCheckSection = hospitalName
    ? ' "병원 내외부" 사진에 병원명(간판/로고 등 글자)이 보이면 그 병원명이 위 "후기 등록 병원명"과 같은 병원인지 확인하세요. 다른 병원이면 이 후기와 무관한 사진이므로 병원 내외부 승인 대상으로 인정하지 말고 시술과 무관한 사진(irrelevant)으로 보류하세요.'
    : '';
...
"병원 내외부"로 승인하려면 사진에 시술 배너, 병원명(간판/로고), 의료기기, 약품, 파우더룸, 시술 관련 팜플렛/안내문, 의료진(의사·간호사 등) 중 하나 이상이 나와서 병원임을 알아볼 수 있어야 합니다. 이런 요소 없이 복도·천장·바닥·일반 가구·커튼 등 병원인지 특정할 수 없는 공간만 나온 사진은 시술과 무관한 것으로 보고 보류하세요. 리셉션 데스크나 대기실 소파처럼 그 자체만으로는 병원 여부를 특정할 수 없는 공간도 위 요소가 함께 나오지 않으면 보류하세요.${hospitalNameCheckSection}
